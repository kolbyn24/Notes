...
## Description:  
Injecting into processes that typically do not generate network traffic can we detected. We can migrate to svchost.exe, which normally generates network activity. However, The problem is that all svchost.exe processes run by default at SYSTEM integrity level, meaning we cannot inject into them from a lower integrity level. Additionally, if we were to launch svchost.exe (instead of Notepad) and attempt to inject into it, the process will immediately terminate.
...
Created Date: December 28th 2022 (11:45 am)  
